 ...
 */
export function processNovelLines(body, options = {}) {
    // 行に分割して処理（元の改行単位）
 ...
 */
export function reconstructNovelText(processedLines, options = {}) {
    if (processedLines.length === 0)
        return "";
    const emptyLinesToSeparator = (emptyLines) => "\n".repeat(emptyLines + 1);
    let result = "";
    for (let i = 0; i < processedLines.length; i++) {
        ...
        if (i === 0) {
            ...
            const isDialogueBoundary = !!prev && !current.isSeparator && !prev.isSeparator && current.isDialogue !== prev.isDialogue;
            const isDialogueToDialogue = !!prev && !current.isSeparator && !prev.isSeparator && current.isDialogue && prev.isDialogue;
