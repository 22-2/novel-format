 ...
 */
export function preprocessMarkdown(body, separator = SECTION_SEPARATOR) {
    ...
    function isParent(idx) {
        const cur = meta[idx];
        for (let j = idx + 1; j < meta.length; j++) {
            const nxt = meta[j];
            if (nxt.isBlank)
                continue;
            if (nxt.isList && nxt.indent > cur.indent)
                return true;
            break;
        }
        return false;
    ...
    function flushGroup() {
        ...
        // 先頭の空エントリを削除
        let a = 0;
        while (a < groupBuffer.length && groupBuffer[a] === null)
            a++;
        // 末尾の空エントリを削除
        let b = groupBuffer.length - 1;
        while (b >= a && groupBuffer[b] === null)
            b--;
